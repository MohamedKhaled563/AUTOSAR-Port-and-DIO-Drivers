...
void Port_Init( const Port_ConfigType* ConfigPtr )
{
   ...
   * development error PORT_E_PARAM_CONFIG and return without any action.*/
...
#endif 
    {
        ...
        for (uint8 portPinIndex = PORTCONFIG_A_PIN_0_ID_INDEX; portPinIndex < PORT_PINS_CONFIGURED_NUMBER; portPinIndex++)
        {
             ...
             || ((Port_pins[portPinIndex].port_num == PORTCONFIG_PORT_F_NUM) && (Port_pins[portPinIndex].pin_num == PORTCONFIG_PIN_0_NUM)) ) /* PD7 or PF0 */
            {
                *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_LOCK_REG_OFFSET) = 0x4C4F434B;                     /* Unlock the GPIOCR register */   
                SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_COMMIT_REG_OFFSET) , Port_pins[portPinIndex].pin_num);  /* Set the corresponding bit in GPIOCR register to allow changes on this pin */
            }
            /* JTAG Pins */
            ...
            {
                /* Clear Analog Mode only for pins with digital mode */
...
void Port_SetPinDirection( Port_PinType Pin, Port_PinDirectionType Direction )
{
    ...
    if( Pin >= PORT_PINS_CONFIGURED_NUMBER )
    {
        Det_ReportError(PORT_MODULE_ID, PORT_INSTANCE_ID, PORT_SET_PIN_DIRECTION_SID, 
                        PORT_E_PARAM_PIN);
        error = TRUE;
...
void Port_SetPinMode( Port_PinType Pin, Port_PinModeType Mode )
{
    ...
    if( FALSE == error )
    {
        ...
        /* Set Analog mode and Digital Mode registers */
        if( PORTCONFIG_PIN_MODE_ANALOG == Port_pins[Pin].mode )
        {
            /* Set Analog Mode only for pins with analog mode */
        ...
        *(volatile uint32 *)((volatile uint8 *) PortGpio_Ptr + PORT_CTL_REG_OFFSET) &= ~(0x0000000F << Port_pins[Pin].pin_num * 4);
        ...
        /* Clear Alternative Function if GPIO */
        if( (Port_pins[Pin].mode == PORTCONFIG_PIN_MODE_DIO) || (Port_pins[Pin].mode == PORTCONFIG_PIN_MODE_ANALOG) )
        {
